...
- **Execution trace player.** The visualizer can run *your* code on the shown sample through the local CPOS runner and animate its execution on the drawing: print `#cpos visit u` / `#cpos set i v` / `#cpos cell r c v` / `#cpos edge u v` / `#cpos frame` lines to stderr (2-line macro provided in the panel's ? help) and scrub your own BFS/DFS/DP frame by frame — in the browser it runs the in-browser editor's code, in VS Code the active solution file. **Zero setup needed**: the runner auto-traces your program — Python/PyPy runs under a line tracer that narrates every new variable, changed value and list/matrix mutation; C++ sources get array/matrix assignments, scalar (re)assignments and new declarations instrumented automatically before compiling (with silent fallback to the original source if instrumentation cannot compile). A live variables panel shows watched values as the trace plays, an execution cursor rings whatever the current frame just touched, and index variables (i, j, l, r, u…) are drawn as labeled pointers under the exact cell or node they currently reference — C++ loop headers are instrumented so the loop variable is always tracked; `#cpos` lines remain available for manual control. Run results and errors flash on the canvas, multi-test inputs execute exactly the case being viewed, and keyboard macros drive the panel (arrow keys or [ ] / { } for samples and cases — with ‹ › buttons beside the case selector — space/,/. for trace playback, r to run). Expected-output views understand answer shapes too: binary/leading-zero masks and YES/NO lines render as string rows, integer columns as arrays. A marker tool (✏ or m) draws freehand over any visualization in three ink colors — strokes survive pan/zoom, are included in SVG export, and clear when the structure changes.
